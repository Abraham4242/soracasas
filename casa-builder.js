...
const SCENERY_POS=[[-22,9],[24,-14],[-26,-18],[28,13],[-17,-24],[20,20],[-34,2],[34,-3],[-12,30],[15,-32],[-42,-20],[42,18]];
export function buildScenery(scn,n){const g=new THREE.Group();const k=Math.max(1,Math.min(n||6,SCENERY_POS.length));for(let i=0;i<k;i++){const[x,z]=SCENERY_POS[i];g.add(scn==='beach'?palm(x,z):pine(x,z));}return g;}
...
export function makeGround(){
  ...
  return {mesh,set(scn,tint){const src=scn==='beach'?B.sand:B.grass;const tm=src.clone();tm.wrapS=tm.wrapT=THREE.RepeatWrapping;tm.repeat.set(scn==='beach'?40:50,scn==='beach'?40:50);groundMat.map=tm;groundMat.color.set(tint||0xffffff);groundMat.needsUpdate=true;}};
}

// distant geography per region: a Barú-style volcano for the highlands, sea around a coastal lot
function hills(g){const m=new THREE.MeshStandardMaterial({color:0x6f8d54,roughness:1});[[-78,-92,34],[72,-98,38],[8,-128,46]].forEach(([x,z,r])=>{const h=new THREE.Mesh(new THREE.SphereGeometry(r,28,18,0,Math.PI*2,0,Math.PI*0.5),m);h.position.set(x,-3,z);g.add(h);});}
export function buildBackdrop(kind){
  const g=new THREE.Group();
  if(kind==='volcano'){
    const lower=new THREE.Mesh(new THREE.ConeGeometry(52,34,44),new THREE.MeshStandardMaterial({color:0x5e7048,roughness:1}));lower.position.set(-26,15,-118);g.add(lower);
    const upper=new THREE.Mesh(new THREE.ConeGeometry(26,22,40),new THREE.MeshStandardMaterial({color:0x6b6256,roughness:1}));upper.position.set(-26,34,-118);g.add(upper);
    const cap=new THREE.Mesh(new THREE.ConeGeometry(10,9,30),new THREE.MeshStandardMaterial({color:0xeef0ef,roughness:.9}));cap.position.set(-26,47,-118);g.add(cap);
    hills(g);
  } else if(kind==='hills'){ hills(g); }
  else if(kind==='ocean'||kind==='caribbean'){
    const col=kind==='caribbean'?0x39b3c2:0x2d7d96;
    const water=new THREE.Mesh(new THREE.PlaneGeometry(1400,1400),new THREE.MeshStandardMaterial({color:col,roughness:.18,metalness:.08}));
    water.rotation.x=-Math.PI/2;water.position.set(0,-0.9,0);g.add(water);
  }
  return g;
}
